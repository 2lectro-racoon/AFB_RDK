...
# RDK X5 note:
# - Verified working SPI node for this project is /dev/spidev1.0
# - Python test used: spi.open(1, 0), max_speed_hz=500000, mode=0
# - Keep these defaults aligned with the confirmed STM32 connection on RDK X5.

# -------------------- SPI config --------------------
SPI_BUS = 1
SPI_DEVICE = 0  # CE0 (/dev/spidev1.0 on RDK X5)
...
def get_spi() -> spidev.SpiDev:
    ...
    if _spi is None:
        s = spidev.SpiDev()
        # Opens the confirmed RDK X5 SPI device (/dev/spidev1.0)
...
def get_mode() -> List[int]:
    """Read current firmware mode/status via CMD 0x00."""
